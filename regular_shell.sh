...
# shellcheck source=/dev/null
[[ -e "$DOTFILE_PARENT_PATH/dotfiles/_ggc" ]] && source "$DOTFILE_PARENT_PATH/dotfiles/_ggc"
...
if [[ "$(uname -r)" == *WSL2 ]]; then
  # shellcheck source=/dev/null
  [[ -e "$DOTFILE_PARENT_PATH/dotfiles/_wsl.sh" ]] && source "$DOTFILE_PARENT_PATH/dotfiles/_wsl.sh"
fi
